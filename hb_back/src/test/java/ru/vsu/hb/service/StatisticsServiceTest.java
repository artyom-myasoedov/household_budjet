...
class StatisticsServiceTest {
    ...
    public void getSumRecommendationTest() {
        ...
        assertTrue(st.getRecommendations().contains("Вы привысили расходы в текущем месяце в сравнении с предыдущим на 1.00 руб. (25.00%)."));
        assertTrue(st.getRecommendations().contains("В текущем месяце у вас появились расходы по категории \"1\", в отличие от предыдущего месяца. Возможно стоит подумать, действительно ли они необходимы."));
        assertTrue(st.getRecommendations().contains("В текущем месяце у вас отсутствуют расходы по категории \"4\", в отличие от предыдущего месяца. Продолжайте в том же духе!"));
